...
gelf.setConfig = function (conf) {
  if (conf.aliases) {
    this.bindAliases(conf.aliases, this.config.aliases);
  }
...
gelf.bindAliases = function (entriesToAdd, entriesToRemove) {
  var self = this;
  ...
  _(entriesToRemove).omit(blacklist).toPairs().forEach(function (e) {
    self[e[0]] && delete self[e[0]];
  });
  _(entriesToAdd).omit(blacklist).toPairs().forEach(function (e) {
    Object.defineProperty(self, e[0], {enumerable: true, value: self[e[1]]});
  });
  return this;
}
