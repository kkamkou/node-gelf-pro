...
gelf.setConfig = function (conf) {
  if (conf.aliases) {
    this.bindAliases(conf.aliases, _.keys(this.config.aliases));
  }
...
gelf.bindAliases = function (aliasMapping, aliasesToRemove) {
  var self = this;
  var blacklist = _(self.levels).keys().concat([
    'config', 'levels', 'adapter', 'getadapter', 'getstringfromobject', 'send',
    'setconfig', 'message', 'bindaliases'
  ]).value();

  _(aliasesToRemove).omit(blacklist).forEach(function (alias) {
    self[alias] && delete self[alias];
  });
  _(aliasMapping).toPairs().filter(function (p) {
    return !_.includes(blacklist, p[0]);
  }).forEach(function (p) {
    Object.defineProperty(self, p[0], {enumerable: true, value: self[p[1]]});
  });
  return this;
}

// defining default functions like info(), error(), etc.
_.forEach(gelf.levels, function (idx, lvl) {
  this[lvl] = function (message, extra, cb) {
    this.message(message, idx, extra, cb);
  };
}.bind(gelf));

module.exports = gelf.bindAliases(gelf.config.aliases);
